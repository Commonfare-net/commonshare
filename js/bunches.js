...
var bunchchart = d3.select("#bunches").attr("clip-path","url(#my-clip)");
var labelsg = d3.select("#bunchlabels").append("g").attr("class", "bunchlabels");
...
var bunchg = bunchchart.append("g").attr("class", "bunchpack");
...
function plotbunches(mydata) {
    ...
    var bunchnode = bunchg.append("g").attr("id", "bunchg").attr("stroke", "#fff").attr("stroke-width", 1.5).selectAll(".node");
    bunchchart.call(bunchzoom);
    var bunchlinks = bunchg.append("g").attr("id", "bunchlinks").selectAll(".link");

    bunchnode = bunchnode.data(mydata.nodes.filter(function(d){return d.type != "tag";}), function (d) {
        return d.id;
    });
    ...
    //        infotooltip.style("background", "lightsteelblue");
            d3.select(n[i]).select("circle").style("filter","url(#glow)");
        ...
        .on("mouseout", function (d,i,n) {
            d3.select(n[i]).select("circle").style("filter","");
    ...
    bunchcircles = bunchnode
        ...
        .attr("stroke-width", "3");
    bunchlinks = bunchlinks.data(mydata.links.filter(function(d){return "edgemeta" in d;}), function (d) {
    ...
    var simulation = d3.forceSimulation()
        ...
        .nodes(mydata.nodes.filter(function(d){return d.type != "tag";}))
    ...
    var blabels = labelsg.append("text")
        ...
        .on("mouseover", function (d,i,n) {
            d3.select(n[i]).style("cursor", "pointer");
            d3.select(n[i]).style("fill", "#E7472E");
        })
        .on("mouseout", function (d,i,n) {
            d3.select(n[i]).style("fill", nodecolor("commoner"));
        })
        ...
        .on("mouseout", function (d,i,n) {
            d3.select(n[i]).style("fill", nodecolor("listing"));
        })
    ...
    var bsociallabels = labelsg.append("text")
        ...
        .on("mouseout", function (d,i,n) {
            d3.select(n[i]).style("fill", nodecolor("story"));
        })
        .style("font-size", "18px")
