...
function sumStats(stats){
    ...
    var startYear = new Date(2018,5,1);
    var cellSize = 28;
...
function zoomFunction(){
    ...
    var formatDay = d3.timeFormat("%w"), // day of the week
        ...
        formatMyFormat = d3.timeFormat("%d/%m/%y");
  ...
  var keys = []
    ...
    var rect = calendar.selectAll(".rect")
        .data(function(d){
            ...
            var mapped = d3.timeDays(new Date(year,month,0), new Date(year,month+1,1)).map((val, index, arr) => {   
                if(d.stats == null)
  ...
  .range(['#a6cee3','#1f78b4','#b2df8a','#33a02c']);
    ...
    .attr("stroke", "#000")
    .selectAll("path")
    .data(function(d) { return d3.timeMonths(new Date(2018, 5, 1), new Date(2019, 4, 1)); })
    .enter().append("path")
    .attr("d", pathMonth);
