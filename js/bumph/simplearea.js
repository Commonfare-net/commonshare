function plotsimplearea(user) {
	...
	var zoom = d3.zoom()
		.scaleExtent([0.5, 3])
		.translateExtent([[0, 0], [chart.attr('width') * 2 + 50, chart.attr("height")]])
		.on('zoom', zoomed);
	...
	function zoomed() {
		...
		if (tickdistance < 50)
			...
			currentMonthGap = Math.max(1,--currentMonthGap);
                console.log("mongap: " + currentMonthGap);
	...
	for (var month in data) {
        ...
        console.log(node_data[month]);
		var cumu_totals = node_data[month].cumu;
        if(cumu_totals == null){
          cumu_totals = {};
          cumu_totals["date"] = node_data[month].date;
          cumu_totals["stats"] = {};
          for(var name in keys){
              cumu_totals[keys[name]] = 0;
          }
